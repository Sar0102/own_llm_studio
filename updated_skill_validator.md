...
### ⚠️ CRITICAL CONTEXT CONSTRAINT

**Never load more than one source document into context at a time.**
Raw document content must never accumulate across steps.
All inter-document analysis is done exclusively through compact `tmp/analysis_*.json` files, not through re-reading original documents.
...
1. Write the file list to `tmp/file_list.json`:
...
### Phase 1: Per-File Analysis Loop

**⚠️ Process strictly ONE file at a time. Do not read the next file until the current one is fully analyzed and saved.**

For **each file** in `tmp/file_list.json`:

**Step 1. Read current file**

Read the content of this single file using `read_file`.

**Step 2. Analyze current file**

While the file content is in context, extract:

- Document type (from the Document Types table above)
- Sections present (headings hierarchy)
- Sections missing (compare against Required Sections for this doc type)
- Key terms (product-specific terminology, component names)
- Components list (named system components)
- SPO list (software listed in system requirements)
- Version numbers mentioned
- Cross-reference targets (other documents or sections referenced)
- Per-file validation issues: missing sections, broken references, note violations

**Step 3. Save compact analysis — immediately after Step 2**

Write result to `tmp/analysis_{doc_type}.json` using `write_file`:
...
**Step 4. Discard file content**

Do NOT reference or re-read the raw file content after this step.
...
**⚠️ Read ONLY `tmp/analysis_*.json` files in this phase. Never re-read original documents.**

**Step 5. Load all compact analyses**

Read all `tmp/analysis_*.json` files. These are small and safe for context.
...
Merge all `issues` arrays from `tmp/analysis_*.json` with cross-document issues from Steps 6–7.

Save to `{workspace_path}/reports/consistency-validator.json`.

**Step 9. Cleanup**

Delete all `tmp/analysis_*.json` and `tmp/file_list.json` files.
...
1. **Never re-read raw document files during Phase 2** — use only `tmp/analysis_*.json`
