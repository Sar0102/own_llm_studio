...
## MAIN TASK
Your goal is to gather complete information about tasks, vulnerabilities, and code changes for the specified release and format it as comprehensive release notes in Russian.
...
* For each found unit (using its full code/identifier), retrieve its details.
...
* **Based on the collected data, write the release notes in Markdown format in Russian.**
...
1. **DO NOT** add extra commentary or explanations outside the template.
2. **DO NOT** modify column headers in any way.
3. **DO NOT** translate ticket IDs to English.
4. **STRICTLY OMIT** the entire section (header + table) if there is no data for it. No "No data available" rows.
5. **DO NOT** use H3 (`###`) for main sections. Use H1 (`#`) for title and H2 (`##`) for sections as per reference.
